...
router.get("/chat", isLoggedIn, async function (req, res) {
  try {
    let user = await userModel.findOne({ username: req.user.username }).populate("posts").populate("saved");
    console.log(user);
    res.render("chat", { footer: true, user });
  } catch (error) {
    console.error("Error fetching user data:", error);
    res.redirect("/login"); // Redirect to login if user data cannot be fetched
  }
});
...
router.get("/feed2", isLoggedIn, async function (req, res) {
